...
class ConstantOperator(OperatorBase):
    ...
    def apply(self, U, mu=None):
        assert U in self.source
        return self._value.copy(ind=([0] * len(U)))
...
class VectorArrayOperator(OperatorBase):
    ...
    def apply_adjoint(self, U, mu=None, source_product=None, range_product=None):
        ...
        if not self.transposed:
            if range_product:
                ATPrU = NumpyVectorArray(range_product.apply2(self._array, U).T, copy=False)
            else:
                ATPrU = NumpyVectorArray(self._array.dot(U).T, copy=False)
            if source_product:
